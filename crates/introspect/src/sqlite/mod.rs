...
fn read_table(connection: &Connection, entry: &CatalogTable) -> rusqlite::Result<Table> {
    ...
    let catalog_primary_key = if primary_key_columns.is_empty() {
        ...
        vec![Constraint {
            ...
            expression: None,
            references: None,
            backend: sqlite_constraint(None, false, false),
        }]
    };
    ...
    Ok(Table {
        ...
        backend: TableBackend::Sqlite(SqliteTable {
            ...
            kind: match &entry.kind {
                CatalogTableKind::Ordinary => SqliteTableKind::Ordinary,
                CatalogTableKind::Virtual => SqliteTableKind::Virtual {
                ...
                CatalogTableKind::Shadow { virtual_table } => SqliteTableKind::Shadow {
...
fn parse_foreign_key_reference(
    ...
    let mut reference = ForeignKeyReference {
        namespace: namespace.to_string(),
        table: sqlite_identifier(clause.tbl_name.0),
        columns: clause
            .columns
            .map(|columns| {
                columns
        ...
        match_name: None,
        deferrability: parse_deferrability(defer_clause),
    };
...
fn parse_deferrability(clause: Option<&DeferSubclause>) -> ForeignKeyDeferrability {
    clause.map_or_else(ForeignKeyDeferrability::default, |clause| {
        ForeignKeyDeferrability {
            ...
            initially: match clause.init_deferred {
...
fn new_sqlite_constraint(
    ...
    Constraint {
        ...
        expression,
        references,
        backend: ConstraintBackend::Sqlite(sqlite),
    }
}
...
fn sqlite_constraint(
    conflict_resolution: Option<SqliteConflictResolution>,
    auto_increment: bool,
    declared_on_column: bool,
) -> ConstraintBackend {
    ConstraintBackend::Sqlite(SqliteConstraint {
        conflict_resolution,
        auto_increment,
        declared_on_column,
    })
...
fn parse_conflict_resolution(resolution: ResolveType) -> SqliteConflictResolution {
    match resolution {
        ResolveType::Rollback => SqliteConflictResolution::Rollback,
        ResolveType::Abort => SqliteConflictResolution::Abort,
        ResolveType::Fail => SqliteConflictResolution::Fail,
        ResolveType::Ignore => SqliteConflictResolution::Ignore,
        ResolveType::Replace => SqliteConflictResolution::Replace,
    }
}
...
fn read_indexes(
    ...
    for catalog_index in catalog_indexes {
        ...
        indexes.push(Index {
            ...
            predicate,
            definition,
            backend: IndexBackend::Sqlite(SqliteIndex {
...
fn sqlite_index_origin(value: &str, column_index: usize) -> rusqlite::Result<SqliteIndexOrigin> {
    match value {
        "c" => Ok(SqliteIndexOrigin::CreateIndex),
        "u" => Ok(SqliteIndexOrigin::UniqueConstraint),
        "pk" => Ok(SqliteIndexOrigin::PrimaryKey),
...
fn sqlite_column_kind(value: u32) -> rusqlite::Result<SqliteColumnKind> {
    match value {
        0 => Ok(SqliteColumnKind::Normal),
        1 => Ok(SqliteColumnKind::VirtualTableHidden),
        2 => Ok(SqliteColumnKind::VirtualGenerated),
        3 => Ok(SqliteColumnKind::StoredGenerated),
        value => Err(rusqlite::Error::FromSqlConversionFailure(
            5,
            Type::Integer,
