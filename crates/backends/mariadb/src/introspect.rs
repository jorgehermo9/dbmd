...
use super::{
    ...
    Routine, Schema, Sequence, Snapshot, SystemTimePeriod, Table, Trigger, View,
};
...
fn load_schemas(
    ...
    let values = rows(connection, source, "schemas", &format!("SELECT SCHEMA_NAME, DEFAULT_CHARACTER_SET_NAME, DEFAULT_COLLATION_NAME FROM information_schema.SCHEMATA WHERE {} ORDER BY SCHEMA_NAME", schema_filter(source, "SCHEMA_NAME")))?;
    values
        .into_iter()
        .map(|row| {
            Ok(Schema {
...
fn attach_indexes(
    ...
    let values = rows(connection, source, "indexes", &format!("SELECT TABLE_SCHEMA, TABLE_NAME, INDEX_NAME, NON_UNIQUE, SEQ_IN_INDEX, COLUMN_NAME, COLLATION, SUB_PART, INDEX_TYPE, COMMENT, IGNORED FROM information_schema.STATISTICS WHERE {} ORDER BY TABLE_SCHEMA, TABLE_NAME, INDEX_NAME, SEQ_IN_INDEX", schema_filter(source, "TABLE_SCHEMA")))?;
...
fn attach_partitions(
    ...
    let values = rows(connection, source, "partitions", &format!("SELECT TABLE_SCHEMA, TABLE_NAME, PARTITION_NAME, SUBPARTITION_NAME, PARTITION_METHOD, PARTITION_EXPRESSION, PARTITION_DESCRIPTION, PARTITION_ORDINAL_POSITION FROM information_schema.PARTITIONS WHERE PARTITION_NAME IS NOT NULL AND {} ORDER BY TABLE_SCHEMA, TABLE_NAME, PARTITION_ORDINAL_POSITION, SUBPARTITION_ORDINAL_POSITION", schema_filter(source, "TABLE_SCHEMA")))?;
    let mut grouped = BTreeMap::new();
    for row in values {
        grouped
            .entry((row.required("TABLE_SCHEMA")?, row.required("TABLE_NAME")?))
...
fn load_views(
    ...
    let values = rows(connection, source, "views", &format!("SELECT TABLE_SCHEMA, TABLE_NAME, VIEW_DEFINITION, CHECK_OPTION, IS_UPDATABLE, SECURITY_TYPE, DEFINER, CHARACTER_SET_CLIENT, COLLATION_CONNECTION FROM information_schema.VIEWS WHERE {} ORDER BY TABLE_SCHEMA, TABLE_NAME", schema_filter(source, "TABLE_SCHEMA")))?;
    values
        .into_iter()
        .map(|row| {
            ...
            let create_statement = rows(connection, source, "view definitions", &sql)?
                .into_iter()
                .next()
...
fn load_sequences(
    ...
    let values = rows(connection, source, "sequences", &format!("SELECT TABLE_SCHEMA, TABLE_NAME FROM information_schema.TABLES WHERE TABLE_TYPE = 'SEQUENCE' AND {} ORDER BY TABLE_SCHEMA, TABLE_NAME", schema_filter(source, "TABLE_SCHEMA")))?;
    values
        .into_iter()
        .map(|row| {
            let schema: String = row.required("TABLE_SCHEMA")?;
            ...
            let definition = rows(connection, source, "sequence definitions", &sql)?
                .into_iter()
                .next()
                .map(|value| value.required_at(1, "create statement"))
                .transpose()?
...
fn load_routines(
    ...
    let values = rows(connection, source, "routines", &format!("SELECT ROUTINE_SCHEMA, ROUTINE_NAME, ROUTINE_TYPE, DTD_IDENTIFIER, ROUTINE_DEFINITION, IS_DETERMINISTIC, SQL_DATA_ACCESS, SECURITY_TYPE, DEFINER, ROUTINE_COMMENT FROM information_schema.ROUTINES WHERE {} ORDER BY ROUTINE_SCHEMA, ROUTINE_NAME, ROUTINE_TYPE", schema_filter(source, "ROUTINE_SCHEMA")))?;
    values
        .into_iter()
        .map(|row| {
            Ok(Routine {
...
fn load_triggers(
    ...
    let values = rows(connection, source, "triggers", &format!("SELECT TRIGGER_SCHEMA, TRIGGER_NAME, EVENT_OBJECT_TABLE, EVENT_MANIPULATION, ACTION_TIMING, ACTION_ORIENTATION, ACTION_STATEMENT, ACTION_ORDER, SQL_MODE, DEFINER FROM information_schema.TRIGGERS WHERE {} ORDER BY TRIGGER_SCHEMA, TRIGGER_NAME", schema_filter(source, "TRIGGER_SCHEMA")))?;
    values
        .into_iter()
        .map(|row| {
            Ok(Trigger {
